...
export class RequestsInterceptor {
  ...
  async setup(webext) {
    ...
    ['onBeforeSendHeaders'].forEach(stage => {
      const setting = settings.intercept.webRequest[stage];
      const scriptExecutor = webext.extensionProperties.get({
        key: setting.getKey(),
  ...
  setupInterception(stages) {
    ...
    (stages || ['onBeforeSendHeaders']).forEach(stage => {
      ...
      // requests.
      const listener = self.listeners[stage];
      const wstage = browser.webRequest[stage];
      ...
      if (settings.inner.perKey[`intercept.webRequest.${stage}.enabled`].getValue()) {
        ...
        const requestTypes = settings.inner.perKey[`intercept.webRequest.${stage}.requestTypes`].getValues();
